var model = function(db) {
	...
	self.post = function(data,callback) {
		// Execute SQL Query
		var query = "insert into contact_numbers set ?";
	...
	self.patch = function(contactId,data,callback) {
		// Execute SQL Query
		var query = 'update contact_numbers set ? where id = ?';
		var params=[ 
			data,
			contactId
		]
	...
	self.delete = function(contactId,data,callback){
		...
		var query = 'delete from contact_numbers where id = ? limit 1';
		var params=[
			contactId,
		]
		console.log(db.format(query,params))
		db.query(query,params,function(err,data){
			if (err)
				...
				callback({status:202,message:"Contact Number Deleted"})
			}
		})
	}
